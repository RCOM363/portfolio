...
The above code doesn't log the data because fetch which is a longer running task has not finished processing yet so it doesn't return an value immediately rather it return a `Promise`.
...
Let's understand how JS executes code step by step
